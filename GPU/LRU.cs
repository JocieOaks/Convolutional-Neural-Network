    ...
    internal class LRU
    {
        ...
        public (uint, MemoryBuffer) AllocateEmpty<T>(Cacheable cacheable, int length, Accelerator accelerator) where T : unmanaged
        {
            ...
            lock (this)
            {
                ...
                Caches.TryAdd(id, new Cache(buffer, new WeakReference<Cacheable>(cacheable)));
                _lru.Enqueue(id);
            }
            AddLiveTask();
            return (id, buffer);
        ...
        public (uint, MemoryBuffer) Allocate<T>(Cacheable Cacheable, T[] values, Accelerator accelerator) where T : unmanaged
        {
            uint id = GenerateId();
            MemoryBuffer1D<T, Stride1D.Dense> buffer;
        ...
        public (uint, MemoryBuffer) UpdateBuffer<T>(Cacheable cacheable, T[] values, Accelerator accelerator) where T : unmanaged
        {
            // If not cached
            uint id = cacheable.ID;
