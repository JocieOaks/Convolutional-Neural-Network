...
public class ConvolutionalLayer : Layer
{
    ...
    public ConvolutionalLayer(int kernalSize, int stride, ref FeatureMap[,] input) : base(kernalSize, stride, ref input)
    {
        ...
        _kernals = new Color[_inputDimensions][];
        _kernalGradient = new float[_inputDimensions][];

        float variance = 0.333f / (_inputDimensions * kernalSize * kernalSize);
    ...
    public override FeatureMap[,] Backwards(FeatureMap[,] inputs, FeatureMap[,] inGradients, float learningRate)
    {
        using (Context context = Context.Create(builder => builder.Cuda()))
        {
            ...
            MemoryBuffer1D<float, Stride1D.Dense>[,] deviceOutGradients = new MemoryBuffer1D<float, Stride1D.Dense>[_inputDimensions, inputs.GetLength(1)];
            MemoryBuffer1D<float, Stride1D.Dense>[] deviceKernalGradients = new MemoryBuffer1D<float, Stride1D.Dense>[_inputDimensions];
    ...
    public override FeatureMap[,] Forward(FeatureMap[,] inputs)
    {
        using (Context context = Context.Create(builder => builder.Cuda()))
        {
            ...
            MemoryBuffer1D<Color, Stride1D.Dense>[,] deviceConvoluted = new MemoryBuffer1D<Color, Stride1D.Dense>[_inputDimensions, inputs.GetLength(1)];
